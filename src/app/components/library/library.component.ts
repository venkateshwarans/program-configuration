...
import {
    ResourceService, ToasterService, ConfigService, UtilService, ICaraouselData,
} from '@sunbird/shared';
        ...
        this.pageApiService.getPageData(option)
            .subscribe(data => {
                ...
                if (!this.carouselMasterData.length) {
                    ...
                    this.carouselMasterData.forEach((section: any) => {
                        section.contents.forEach(content => {
                            content.cardImg = content.image;
                        });
                    });
                }
                ...
                //TODO: Needs to map pageSections for cardImage changes
                // this.pageSections = this.pageSections.map(a => a.map(content => ({ ...content, 'cardImg': content.appIcon })));
                // this.pageSections = this.pageSections.map(section =>
                // section.contents.map(content => ({ ...content, 'cardImg': content.appIcon })));
